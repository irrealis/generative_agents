...
def add_period_if_needed(description):
  if not description.endswith('.'):
    description = f'{description}.'
  return description


def format_plan_thought(persona, node):
  description = f'- {node.created.strftime("At %H:%M:%S")}: {persona.name} is thinking about what to do. {node.description}'
  return add_period_if_needed(description)

def format_bootstrap_or_reflection_thought(persona, node):
  description = f'- {node.created.strftime("At %H:%M:%S")}: {persona.name} reflects that {node.description}'
  return add_period_if_needed(description)

def format_chat_event(persona, node):
  lines = []
  line = f'- {node.created.strftime("At %H:%M:%S")}: {node.description}'
  line = add_period_if_needed(line)
  line = f'{line} Here is the dialog history:'
  lines.append(line)
  for chat_node_id in node.filling:
    chat_node = persona.a_mem.id_to_node[chat_node_id]
    for (speaker, dialog) in chat_node.filling:
      lines.append(f'  - {speaker}: {dialog}')
  description = '\n'.join(lines)
  return description

def format_object_observation_event(persona, node):
  match = node.is_object_observation_event()
  world,sector,arena,game_object = match.groups()
  description = f'- {node.created.strftime("At %H:%M:%S")}: In the {arena} of {sector}, {persona.name} is aware that the {node.description}'
  return add_period_if_needed(description)

def format_activity_event(persona, node):
  description = f'- {node.created.strftime("At %H:%M:%S")}: {node.description}'
  return add_period_if_needed(description)


def format_memory_stream(persona):
  node_descriptions = []
  date = None
...
'''
    ...
    if not node.is_idle():
      ...
      if 'plan_thought' in matches:
        descriptions.append(format_plan_thought(persona, node))
      if 'reflection_thought' in matches:
        descriptions.append(format_bootstrap_or_reflection_thought(persona, node))
      if 'reflection_error_thought' in matches:
        pass
      if 'bootstrap_thought' in matches:
        descriptions.append(format_bootstrap_or_reflection_thought(persona, node))
      if 'chat_event' in matches:
        descriptions.append(format_chat_event(persona, node))
      if 'object_observation_event' in matches:
        descriptions.append(format_object_observation_event(persona, node))
      if 'activity_event' in matches:
        descriptions.append(format_activity_event(persona, node))
      if 'chat' in matches:
        pass
...
def get_roleplay_interview_response_prompt(persona, message):
  #memory_stream_text = format_memory_stream(persona)
  memory_stream_text = persona.format_memory_stream()
  prompt = f'''Below are {persona.name}'s recent memories.

{memory_stream_text}

Today is {persona.scratch.get_str_curr_date_str()}. Using the above memories, please roleplay how {persona.name} would respond if asked "{message}"
...
def test_brainstorm__ConceptNode__categorize_memories(rs):
  persona = rs.personas['Isabella Rodriguez']
  prompt = get_roleplay_interview_response_prompt(
    persona,
...
def test_brainstorm__concise_complete_memory_stream(rs):
  persona = rs.personas['Isabella Rodriguez']
