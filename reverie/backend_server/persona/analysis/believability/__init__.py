...
class BelievabilityEvaluator:
  def __init__(
    ...
    interviews,
    personas,
    # FIXME@kaben: Something tells me the following two parameters need to be
    # better thought out.
    llm_parameters = None,
    llm = None,
  ...
  def get_ranking_prompt(
    ...
    (
      ...
      condition_map,
    ) = self.get_shuffled_conditions(interview_question_dict)
  ...
  def generate_persona_dict(self, interview_persona_dict):
    ...
    for interview_category_dict in interview_category_dicts:
      evaluation_category_dict = self.generate_category_dict(
